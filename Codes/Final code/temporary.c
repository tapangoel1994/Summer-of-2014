...
void eval1(double t, double *x_init, double *f, int *circ,double *constants )
{
	int j;
	int i;
	int p=0;
...
void print(int a[],int num)//test code
{
	int i;
	for(i=0;i<num;i++)
		printf("%d ",a[i]);
...
void print(double a[],int num)//test code
{
	int i;
	for(i=0;i<num;i++)
		printf("%lf ",a[i]);
...
int main()
{
	...
	for(i=0;i<2;i++)//loop to open a file in read mode and then an internal loop to run each parameter set based on circuit.
	{
		sprintf(str,"%ld",i);
		f=fopen(str,"r");
		...
		no_of_constants=no_of_edges*2;
		
		constants=new double [no_of_constants];
		
		for(k=0;k<2;k++)  //iterate over  parameter sets
		{
			for(j=0;j<no_of_constants;j++)
			{
				fscanf(f,"%lf",&constants[j]);
			}
			
			deterministicrun(0,10000,x,0);
			
			//	print(constants,no_of_constants);
		}
